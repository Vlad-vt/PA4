...
1. Обчислення матриці обертання:
  -Дані акселерометра та магнітомера використовуються для обчислення напрямків гравітації та магнітного поля.
  -Обчислюється векторний добуток цих напрямків.
  -Розмір векторного добутку і кути між напрямками використовуються для побудови матриці обертання.
2. Визначення кутів орієнтації:
   -Кут крену (roll), кут тангажу (pitch) та кут відхилення (yaw) визначаються з матриці обертання.
