...
* Данные акселерометра и магнитометра используются для обсчёта направлений гравитации и магнитного поля.
* Обсчитывается векторный добуток этих направлений.
* Размер векторного добутку и углы между направлениями используются для построения матрицы вращения.

# Определение углов ориентации:
* Кут крена (roll), кут тангажу (pitch) и кут отхилення (yaw) определяются из матрицы вращения.
* Конвертация углов ориентации в кватернион:
* Расчтанные углы ориентации превращаются в кватернион, который является более удобным способом представления ориентации в 3D-пространстве.

# Фильтрация ориентации:
* Используется комплементарный фильтр для слития данных гироскопа и информации с акселерометра и магнитометра.
* Коэффициенты фильтра gyroCoefficient определяют важность каждого источника данных.
* Полученная ориентация после фильтрации сохраняется в fusedOrientation.

# Применение ориентации:
* Объект поворачивается согласно рассчитанной fusedOrientation.
